...
object GenericStream extends IOApp {
  ...
  def getNextChunkGenericOpt(chunkSize: Int): ResultSetIO[Seq[Row]] =
    FRS.raw { rs =>
      val md = rs.getMetaData
      val ks = (1 to md.getColumnCount).map(md.getColumnLabel).toList
      val ns = (1 to md.getColumnCount).map(md.isNullable).toVector
      var n  = chunkSize
      val b  = Vector.newBuilder[Row]
      while (n > 0 && rs.next) {
        val mb = Map.newBuilder[String, Any]
        ks.zip(ns).foreach({ case (k, nn) => mb += (k -> (if (true) rs.getObject(k) else Option(rs.getObject(k)))) })
        b += mb.result()
        n -= 1
      }
      b.result()
